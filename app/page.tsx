...
export default async function HomePage() {
  ...
  const genreGames = await getTopRatedGamesFromGenre(3, 8);
  return (
    <main className="flex flex-col space-y-10">
      ...
      <div className="flex flex-col space-y-3">
        <h2>{genreGames[0].genre.name}</h2>
        <div className="mx-auto my-10 grid w-4/5 grid-cols-2 gap-5 md:grid-cols-3">
          {genreGames.map((game, index) => (
            <GameCardCover key={index} game={game.game} isCompleted={false}>
              <SearchResultEntryControls userId={userId} game={game.game} />
            </GameCardCover>
          ))}
        </div>
      </div>
